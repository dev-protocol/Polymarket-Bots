...
const init = async () => {
    if (!isDbConnected()) {
        temp_trades = [];
        console.log('Trade Monitor: DB not connected, starting with no saved trades');
        return;
    }
...
const fetchTradeData = async () => {
    try {
        ...
        if (!isDbConnected()) {
            if (trades.length > 0) {
                console.log(`Trade Monitor: ${trades.length} trade(s) from API (DB not connected, not persisting)`);
            }
            return;
        }
